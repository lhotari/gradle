...
class ToolingApiCompatibilitySuiteRunner extends AbstractCompatibilityTestRunner {
    ...
    private static ClassLoader LAST_ACTIVE_TEST_CLASS_LOADER;
    ...
    private class Permutation extends AbstractMultiTestRunner.Execution {
        ...
        private void manageCacheInPreviousClassLoader(ClassLoader classLoader) {
            if (classLoader != LAST_ACTIVE_TEST_CLASS_LOADER && LAST_ACTIVE_TEST_CLASS_LOADER != null) {
                clearCacheInPreviousClassLoader()
            }
            LAST_ACTIVE_TEST_CLASS_LOADER = classLoader
        }

        private void clearCacheInPreviousClassLoader() {
            try {
                LAST_ACTIVE_TEST_CLASS_LOADER.loadClass("org.gradle.tooling.internal.consumer.ConnectorServices").newInstance().reset()
            } catch (e) {
                // ignore errors
            }
        }
