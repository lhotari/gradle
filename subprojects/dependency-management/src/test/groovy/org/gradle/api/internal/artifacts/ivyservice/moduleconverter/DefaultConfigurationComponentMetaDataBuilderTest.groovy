...
class DefaultConfigurationComponentMetaDataBuilderTest extends Specification {
    ...
    private Configuration createNamesAndExtendedConfigurationStub(final String name, final Configuration... extendsFromConfigurations) {
        ...
        stub.getArtifacts() >> new DefaultPublishArtifactSet("foo", WrapUtil.toDomainObjectSet(PublishArtifact.class), TestFiles.resolver().getPatternSetFactory())
        stub.getAllArtifacts() >> new DefaultPublishArtifactSet("foo", WrapUtil.toDomainObjectSet(PublishArtifact.class), TestFiles.resolver().getPatternSetFactory())
        return stub;
    }
