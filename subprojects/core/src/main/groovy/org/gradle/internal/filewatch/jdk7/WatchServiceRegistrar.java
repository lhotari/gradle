...
class WatchServiceRegistrar implements FileWatcherListener {
    ...
    private ImmutableCollection<File> currentRoots;
    ...
    void watch(FileSystemSubset fileSystemSubset) throws IOException {
        ...
        try {
            ...
            Iterable<? extends File> startingWatchPoints = calculateStartingWatchPoints(roots);

            final FileSystemSubset unfiltered = fileSystemSubset.unfiltered();

            final ImmutableSet.Builder<File> registeredRoots = ImmutableSet.builder();

            for (File dir : startingWatchPoints) {
                Files.walkFileTree(dir.toPath(), new SimpleFileVisitor<Path>() {
                    @Override
                    public FileVisitResult preVisitDirectory(Path path, BasicFileAttributes attrs) throws IOException {
                        final File subDir = path.toFile();
                        if (inUnfilteredSubsetOrAncestorOfAnyRoot(subDir, roots, unfiltered)) {
                            watchDir(path);
            ...
            if(currentRoots != null) {
                registeredRoots.addAll(currentRoots);
            }

            currentRoots = registeredRoots.build();
        } finally {
            lock.unlock();
        }
    }

    private Iterable<? extends File> calculateStartingWatchPoints(Iterable<? extends File> roots) {
        ...
        // Collapse the set
        return FileUtils.calculateRoots(enclosingDirsThatExist);
    }
