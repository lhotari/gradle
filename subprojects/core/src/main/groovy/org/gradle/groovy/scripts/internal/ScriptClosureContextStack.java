...
public class ScriptClosureContextStack {
    ...
    private static final ThreadLocal<Object> FALLBACK_DYNAMIC_TARGET = new ThreadLocal<Object>();
    private static final ThreadLocal<Set<DynamicObject>> PREVIOUS_DYNAMIC_OBJECTS = new ThreadLocal<Set<DynamicObject>>() {
        @Override
    ...
    public static Object setFallbackDynamicTarget(Object delegate) {
        Object previous = FALLBACK_DYNAMIC_TARGET.get();
        FALLBACK_DYNAMIC_TARGET.set(delegate);
        return previous;
    }
    ...
    private static DynamicObject findParent(Object originalDelegate) {
        ...
        final Set<DynamicObject> usedDynamicObjects = PREVIOUS_DYNAMIC_OBJECTS.get();
        if (current != null) {
        ...
        if (contextStack != null) {
            ...
            if (returnNextFound) {
                Object fallbackTarget = FALLBACK_DYNAMIC_TARGET.get();
    ...
    public static class Holder {
        private static final ThreadLocal<ScriptClosureContextStack> CURRENT_STACK = new ThreadLocal<ScriptClosureContextStack>();

        public static ScriptClosureContextStack create() {
            ScriptClosureContextStack previous = CURRENT_STACK.get();
            CURRENT_STACK.set(new ScriptClosureContextStack());
            return previous;
        }
        ...
        public static void push(Object owner, Object delegate) {
            ScriptClosureContextStack currentStack = CURRENT_STACK.get();
    ...
    private static class ClosureContextDynamicObject extends AbstractDynamicObject {
        ...
        public String getDisplayName() {
            try {
                ...
                PREVIOUS_DYNAMIC_OBJECTS.remove();
            }
        }
