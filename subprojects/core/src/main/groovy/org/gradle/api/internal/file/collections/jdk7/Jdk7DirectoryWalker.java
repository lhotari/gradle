...
public class Jdk7DirectoryWalker implements DirectoryFileTree.DirectoryWalker {
    ...
    @Override
    public void walkDir(final File file, final RelativePath path, final FileVisitor visitor, final Spec<FileTreeElement> spec, final AtomicBoolean stopFlag, final FileSystem fileSystem, final boolean postfix) {
        final ArrayDeque<FileVisitDetails> directoryDetailsHolder = new ArrayDeque<FileVisitDetails>();

        try {
            Files.walkFileTree(file.toPath(), EnumSet.of(FileVisitOption.FOLLOW_LINKS), 512, new java.nio.file.FileVisitor<Path>() {
                @Override
                ...
                private FileVisitDetails getFileVisitDetails(Path file, boolean isFile) {
                    ...
                    RelativePath childPath = (dirDetails != null ? dirDetails.getRelativePath() : path).append(isFile, child.getName());
            ...
            throw new GradleException(String.format("Could not read path '%s'.", file), e);
        }
    }
}
