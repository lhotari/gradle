...
public class CachingPatternSpecFactory extends PatternSpecFactory {
    ...
    public CachingPatternSpecFactory() {
        ...
        int resultsCacheMaxSize = sizer.scaleValue(RESULTS_CACHE_MAX_SIZE);
        specResultCache = CacheBuilder.newBuilder().maximumSize(resultsCacheMaxSize).build();
        int instancesMaxSize = sizer.scaleValue(INSTANCES_MAX_SIZE);
        specInstanceCache = CacheBuilder.newBuilder().maximumSize(instancesMaxSize).build();
    }
