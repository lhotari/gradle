...
public class WorkerAction implements Action<WorkerProcessContext>, Serializable, RequestProtocol {
    private static final int PING_TIMEOUT_MILLIS = 5000;
    ...
    private transient long lastPingMillis;
    ...
    // execution is starting soon, if ping hasn't timed out
    private boolean isExecutingStarting() {
        if (lastPingMillis == 0) {
            return false;
        }
        long sinceLastPing = System.currentTimeMillis() - lastPingMillis;
        if (sinceLastPing >= 0 && sinceLastPing < PING_TIMEOUT_MILLIS) {
            return true;
        }
        return false;
    }
